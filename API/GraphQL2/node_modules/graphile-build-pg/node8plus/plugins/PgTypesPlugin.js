...
var PgTypesPlugin = function PgTypesPlugin(builder, {
  ...
  builder.hook("build", build => {
    ...
    const pgTweakFragmentForTypeAndModifier = (fragment, type, typeModifier = null, resolveData) => {
      ...
      if (tweaker) {
        ...
        if (type.domainBaseType.isPgArray) {
          ...
          const arrayItemTweaker = pgTweaksByTypeIdAndModifer[arrayItemType.id] && pgTweaksByTypeIdAndModifer[arrayItemType.id][domainBaseTypeModifierKey] || pgTweaksByTypeId[arrayItemType.id]; // If it's a domain over a known type array (e.g. `bigint[]`), use
    ...
    */
    ...
    const MacAddr8Type = pgUseCustomNetworkScalars ? stringType(inflection.builtin("MacAddress8"), build.wrapDescription("An 8-byte MAC address.", "type")) : GraphQLString; // pgExtendedTypes might change what types we use for things
...
when (${fragment}) is null then null
  ...
  case when upper(${fragment}) is null then null
      ...
      if (!gqlTypeByTypeIdAndModifier[type.id][typeModifierKey] && type.type === "d" && type.domainBaseTypeId) {
        ...
        const baseInputType = getGqlInputTypeByTypeIdAndModifier(type.domainBaseTypeId, typeModifier); // Hack stolen from: https://github.com/graphile/postgraphile/blob/ade728ed8f8e3ecdc5fdad7d770c67aa573578eb/src/graphql/schema/type/aliasGqlType.ts#L16
  ...
  builder.hook("build", build => {
    ...
    const GraphQLHStoreType = makeGraphQLHstoreType(build, hstoreTypeName); // Now register the hstore type with the type system for both output and input.
