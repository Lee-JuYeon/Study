  ...
  if (isTableLike) {
    ...
    if (proc.returnsSet) {
      ...
      if (isMutation) {
        ...
        type = ConnectionType;
        fieldScope.isPgFieldConnection = true; // We don't return the first value as the value here because it gets
  ...
  return fieldWithHooks(fieldName, ({
    ...
    function makeQuery(parsedResolveInfoFragment, ReturnType, sqlMutationQuery, functionAlias, parentQueryBuilder, resolveContext, resolveInfo) {
      ...
      const query = queryFromResolveData(sqlMutationQuery, functionAlias, resolveData, {
        useAsterisk: !isMutation && (isTableLike || isRecordLike) && (forceList || proc.returnsSet || rawReturnType.isPgArray) && // only bother with lists
        proc.language !== "sql",
    ...
    if (isMutation) {
      ...
      const isNotVoid = String(returnType.id) !== "2278"; // If set then plural name
    ...
    const scalarAwarePg2gql = v => isTableLike ? pg2gql(v, returnType) : { ...v,
